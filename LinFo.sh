...
run_scan_report() {
    ...
    if env | grep -E 'LD_PRELOAD|LD_LIBRARY_PATH|PATH=.*:.*:' &>/dev/null; then
        echo -e "       ${RED}Suspicious variables found${NC}"
    else
...
run_loot_gather() {
    ...
    find /tmp /var/tmp /dev/shm -type d -perm -0002 2>/dev/null | while read -r dir; do
        echo -e "       ${LIGHT_CYAN}💾 $dir${NC}"
    done

    # Mounted filesystems excluding common virtual filesystems
    echo -e "    ${LIGHT_MAGENTA}🧬 Mounted filesystems (targetable):${NC}"
    ...
    for user in $(cut -f1 -d: /etc/passwd); do
        crontab -u "$user" -l 2>/dev/null | grep -vE '^\s*#|^\s*$' && echo -e "      ➥ ${LIGHT_CYAN}$user${NC}"
    done
    ...
    for user in $(cut -f1 -d: /etc/passwd); do
        crontab -u "$user" -l 2>/dev/null | grep -vE '^\s*#|^\s*$' | grep -E '/tmp|/var/tmp|/dev/shm|/home' && echo -e "      ➥ ${LIGHT_CYAN}$user${NC}"
    done
    ...
    for user_home in /home/*; do
        ...
        if [ -f "$histfile" ]; then
            echo -e "      ➥ $(basename "$user_home"):"
            tail -n 10 "$histfile" | sed 's/^/         /'
        fi
    done

    # SUID binaries with detailed listing
    echo -e "    ${LIGHT_MAGENTA}📜 SUID binaries with details:${NC}"
    ...
    echo -e "    🔍 Suspicious shell startup files:"
    ...
    echo -e "    🧪 Recently modified temp binaries:"
    ...
    echo -e "    🔐 Recently modified SUID binaries:"
...
if [[ "$RUN_SCAN" -eq 1 && "$RUN_LOOT" -eq 1 ]]; then
    fullscan_output_handler
